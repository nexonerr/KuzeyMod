...
public class CombustibleFluidGeneratorScreen extends AbstractContainerScreen<CombustibleFluidGeneratorMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(KuzeyMod.MOD_ID, "textures/gui/combustible_fluid_generator_gui.png");
    private FluidStackRenderer renderer;

    public CombustibleFluidGeneratorScreen(CombustibleFluidGeneratorMenu pMenu, Inventory pPlayerInventory, Component pTitle) {
    ...
    private void assignFluidRenderer(){
        renderer = new FluidStackRenderer(KuzeyCommonConfigs.combustibleFluidGeneratorFluidCapacity.get(), true, 16, 41);
    }
    ...
    private void renderEnergyTooltips(PoseStack pPoseStack, int pMouseX, int pMouseY, int x, int y){
        String energyText = GeneralUtils.formatEnergyText(menu.blockEntity.getEnergyStorage().getEnergyStored(),KuzeyCommonConfigs.combustibleFluidGeneratorCapacity.get());
