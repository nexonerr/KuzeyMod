...
public class CombustibleFluidGeneratorMenu extends AbstractContainerMenu implements IFluidMenu, IEnergyMenu {
    public final CombustibleFluidGeneratorBlockEntity blockEntity;
    private final Level level;
    public final ContainerData data;
    private FluidStack fluid;

    public CombustibleFluidGeneratorMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(6));
    }
    public CombustibleFluidGeneratorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.COMBUSTIBLE_FLUID_GENERATOR_MENU.get(), pContainerId);
        checkContainerSize(inv, 1);
        blockEntity = ((CombustibleFluidGeneratorBlockEntity) entity);
        this.level = inv.player.level;
        this.data = data;
        this.fluid = blockEntity.fluidTank.getFluid();

        addPlayerInventory(inv);
        addPlayerHotbar(inv);

        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            this.addSlot(new ModFluidSlot(handler, 0,119,37));
        });
    ...
    public int getEnergyBarScale(){
        ...
        int maxEnergy = KuzeyCommonConfigs.combustibleFluidGeneratorCapacity.get();
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.COMBUSTIBLE_FLUID_GENERATOR.get());
    }
