...
public class EmreEssenceInfuserMenu extends AbstractContainerMenu implements IFluidMenu, IEnergyMenu {
    public final EmreEssenceInfuserBlockEntity blockEntity;
    private final Level level;
    public final ContainerData data;
    private FluidStack fluid;

    public EmreEssenceInfuserMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(7));
    }
    public EmreEssenceInfuserMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.EMRE_ESSENCE_INFUSER_MENU.get(), pContainerId);
        checkContainerSize(inv, 3);
        blockEntity = ((EmreEssenceInfuserBlockEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            this.addSlot(new ModResultSlot(handler,1,82,37));
            this.addSlot(new ModFluidSlot(handler, 2,119,37));
        });
    ...
    public int getEnergyBarScale(){
        ...
        int maxEnergy = KuzeyCommonConfigs.emreEssenceInfuserCapacity.get();
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.EMRE_ESSENCE_INFUSER.get());
    }
