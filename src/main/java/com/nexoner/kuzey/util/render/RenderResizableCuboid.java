...
public class RenderResizableCuboid {
    ...
    public static void renderCube(Model3D cube, PoseStack matrix, VertexConsumer buffer, int[] colors, int light, int overlay, FaceDisplay faceDisplay,
        ...
        for (int y = 0; y <= yDelta; y++) {
            ...
            Model3D.SpriteInfo downSprite = y == 0 ? cube.getSpriteToRender(Direction.DOWN) : null;
            from.setY(yBounds[y]);
            to.setY(yBounds[y + 1]);
            for (int z = 0; z <= zDelta; z++) {
                Model3D.SpriteInfo northSprite = z == 0 ? cube.getSpriteToRender(Direction.NORTH) : null;
                Model3D.SpriteInfo southSprite = z == zDelta ? cube.getSpriteToRender(Direction.SOUTH) : null;
                from.setZ(zBounds[z]);
                to.setZ(zBounds[z + 1]);
                for (int x = 0; x <= xDelta; x++) {
                    Model3D.SpriteInfo westSprite = x == 0 ? cube.getSpriteToRender(Direction.WEST) : null;
                    Model3D.SpriteInfo eastSprite = x == xDelta ? cube.getSpriteToRender(Direction.EAST) : null;
                    //Set bounds
                    from.setX(xBounds[x]);
                    to.setX(xBounds[x + 1]);
     ...
     */
    private static void putTexturedQuad(VertexConsumer buffer, Matrix4f matrix, Matrix3f normalMatrix, @Nullable Model3D.SpriteInfo spriteInfo, Vector3f from, Vector3f to,
